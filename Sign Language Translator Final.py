    ...
    if area_of_circle - area > 33000.0:
        ...
        if 115.0 <= angle_t <= 143.0:
            if 0.96 <= solidity <= 0.99:
                if count_defects <= 1:
                    cv2.putText(frame, "A", (60,50), cv2.FONT_HERSHEY_SIMPLEX, 2 , (0, 0, 0), 2, cv2.LINE_AA)


        if 173.0 <= angle_t <= 180.0:
            if 0.80 <= solidity <= 0.89:
                if count_defects in range(0, 2):
                    cv2.putText(frame, "B", (60,50), cv2.FONT_HERSHEY_SIMPLEX, 2 , (0, 0, 0), 2, cv2.LINE_AA)
        ...
        if 0.0 <= angle_t <= 30.0:
            if count_defects == 1:
                if 0.66 <= solidity <= 0.76:
                    cv2.putText(frame, "C", (60,50), cv2.FONT_HERSHEY_SIMPLEX, 2 , (0, 0, 0), 2, cv2.LINE_AA)
